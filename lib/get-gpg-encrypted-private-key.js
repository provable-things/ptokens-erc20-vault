...
const askUserToSelectPrivateKey = (_choices) =>
  inquirer.prompt({
    type: 'list',
    name: 'selection',
...
const maybeAskUserToSelectPrivateKey = (_choices) =>
  _choices.length === 0
    ? Promise.reject(new Error('✘ No valid private key file found!'))
    : askUserToSelectPrivateKey(_choices)

const getGpgEncryptedPrivateKey = _ =>
  console.info('✔ Getting GPG encrypted private key..') ||
  maybeGetPrivateKeyFiles()
    .then(maybeAskUserToSelectPrivateKey)
