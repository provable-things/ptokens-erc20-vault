...
const pegIn = (
  ...
  getVaultContract(_vaultAddress)
    .then(_vaultContract =>
      Promise.all([ _vaultContract, _vaultContract.signer.getAddress(), _vaultContract.provider ])
    )
    .then(([ _vaultContract, _tokenOwnerAddress, _provider ]) =>
      ...
      Promise.all([
        ...
        _tokenOwnerAddress,
        _provider,
        checkTokenBalanceIsSufficient(_amount, _tokenOwnerAddress, _tokenAddress, _provider)
      ])
    )
    .then(([ _vaultContract, _tokenOwnerAddress, _provider ]) =>
      Promise.all([
    ...
    .then(([ _vaultContract, _provider ]) =>
      Promise.all([
        _vaultContract,
        checkTokenIsSupportedInVault(_vaultAddress, _tokenAddress, _provider),
      ])
    )
    .then(([ _vaultContract ]) =>
